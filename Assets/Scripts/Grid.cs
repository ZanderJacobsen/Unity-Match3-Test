...
namespace Match3
{
    public class Grid<T>
    {
        ...
        public Grid(int width, int height, float cellSize, Vector3 origin, CoordinateConverter coordinateConverter, bool debug)
        {
            this.width = width;
            this.height = height;
        ...
        public static Grid<T> VerticalGrid(int width, int height, float cellSize, Vector3 origin, bool debug = false)
        {
            return new Grid<T>(width, height, cellSize, origin, new VerticalConverter(), debug);
        }

        public static Grid<T> HorizontalGrid(int width, int height, float cellSize, Vector3 origin, bool debug = false)
        {
            return new Grid<T>(width, height, cellSize, origin, new HorizontalConverter(), debug);
        }
        ...
        void DrawdebugLines()
        {
            ...
            for (int x = 0; x < width; x++)
            {
                for (int y = 0; y < height; y++)
                {
                    ...
                    Debug.DrawLine(GetWorldPosition(x + 1, y), GetWorldPosition(x + 1, y), Color.white, duration);
                }
            }
